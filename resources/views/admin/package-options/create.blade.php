...
<div class="db-page-head">
    <div>
        <h1 class="db-page-title">إضافة خيار لباقة: {{ $package->name }}</h1>
        <div class="db-page-subtitle">إنشاء خيار مخصص لهذه الباقة.</div>
    </div>
...
<div class="db-card">
    <div class="db-card-body">
        <form action="{{ route('admin.packages.options.store', $package) }}" method="POST">
            @csrf

            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-12 md:col-span-6 mb-4 mb-3">
                    <label>تسمية الخيار (Label) <span class="text-danger">*</span></label>
                    <input type="text" name="label" class=" @error('label') is-invalid @enderror" value="{{ old('label') }}" required placeholder="مثال: إضافة كاميرا، مدة إضافية...">
                ...
                <div class="col-span-12 md:col-span-3 mb-4 mb-3">
                    ...
                    <select name="type" class=" @error('type') is-invalid @enderror" required>
                        <option value="boolean" {{ old('type') == 'boolean' ? 'selected' : '' }}>نعم/لا (Boolean)</option>
                        <option value="number" {{ old('type') == 'number' ? 'selected' : '' }}>رقمي (Number)</option>
                ...
                <div class="col-span-12 md:col-span-3 mb-4 mb-3">
                    ...
                    <select name="price_effect" class=" @error('price_effect') is-invalid @enderror" required>
                        <option value="fixed" {{ old('price_effect') == 'fixed' ? 'selected' : '' }}>ثابت (Fixed)</option>
                        <option value="per_unit" {{ old('price_effect') == 'per_unit' ? 'selected' : '' }}>لكل وحدة (Per Unit)</option>
                        <option value="free" {{ old('price_effect') == 'free' ? 'selected' : '' }}>مجاني (Free)</option>
                    </select>
                ...
                <div class="col-span-12 md:col-span-6 mb-4 mb-3">
                    ...
                    <input type="number" step="0.01" name="price" class=" @error('price') is-invalid @enderror" value="{{ old('price', 0) }}" required>
                    @error('price') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>

                <div class="col-span-12 md:col-span-3 mb-4 mb-3">
                    <label>الحد الأدنى (للنوع الرقمي)</label>
                    <input type="number" name="min" class=" @error('min') is-invalid @enderror" value="{{ old('min') }}">
                    @error('min') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>

                <div class="col-span-12 md:col-span-3 mb-4 mb-3">
                    <label>الحد الأقصى (للنوع الرقمي)</label>
                    <input type="number" name="max" class=" @error('max') is-invalid @enderror" value="{{ old('max') }}">
                ...
                <div class="col-span-12 mb-4 mb-3">
                    ...
                    <textarea name="options" rows="3" class=" @error('options') is-invalid @enderror" placeholder='[{"name": "اختيار 1", "price": 500}]'>{{ old('options') }}</textarea>
                ...
                <div class="col-span-12 md:col-span-6 mb-4 mb-3">
                    ...
                    <input type="text" name="default_value" class=" @error('default_value') is-invalid @enderror" value="{{ old('default_value') }}">
                    @error('default_value') <span class="invalid-feedback">{{ $message }}</span> @enderror
                </div>

                <div class="col-span-12 md:col-span-6 mb-4 mb-3">
                    <label>الترتيب <span class="text-danger">*</span></label>
                    <input type="number" name="sort_order" class=" @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', 0) }}" required>
                ...
                <div class="col-span-12 mb-4 mb-3 flex items-center gap-4 mt-2">
                    ...
                    <div class="custom-control custom-switch">
                        <input type="checkbox" name="is_required" class="custom-control-input" id="isRequired" value="1" {{ old('is_required') ? 'checked' : '' }}>
            ...
            <div class="flex justify-end">
                <button type="submit" class="db-btn-primary">حفظ الخيار</button>
            </div>
        </form>
    </div>
</div>
