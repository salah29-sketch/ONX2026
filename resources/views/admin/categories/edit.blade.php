...
<div class="db-page-head">
    <h1 class="db-page-title">تعديل تصنيف — {{ $category->name }}</h1>
...
<div class="db-card">
    <div class="db-card-body">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST">
            ...
            <div class="mb-4">
                <label>الاسم *</label>
                <input type="text" name="name" class="db-input" value="{{ old('name', $category->name) }}" required>
                @error('name')<span class="text-danger">{{ $message }}</span>@enderror
            </div>
            <div class="grid grid-cols-12 gap-4">
                <div class="mb-4 col-span-12 md:col-span-6">
                    <label>Slug</label>
                    <input type="text" name="slug" class="db-input" value="{{ old('slug', $category->slug) }}">
                    @error('slug')<span class="text-danger">{{ $message }}</span>@enderror
                </div>
                <div class="mb-4 col-span-12 md:col-span-6">
                    <label>الأيقونة (إيموجي)</label>
                    <input type="text" name="icon" class="db-input" value="{{ old('icon', $category->icon) }}">
                </div>
            </div>
            <div class="mb-4">
                <label>لون التصنيف (يظهر في صفحة الخدمات، الشريط، البطاقات)</label>
                <div class="flex items-center gap-3 flex-wrap">
                    <input type="color" id="cat_bg_picker" value="{{ old('bg_color', $category->bg_color ?? '#E87C2A') }}" class="db-input" style="width:56px;height:40px;padding:2px;cursor:pointer;">
                    <input type="text" name="bg_color" id="cat_bg_text" class="db-input" value="{{ old('bg_color', $category->bg_color ?? '#E87C2A') }}" maxlength="7" placeholder="#E87C2A" style="max-width:120px;direction:ltr;font-family:monospace;">
                </div>
            ...
            <div class="mb-4">
                <label>الوصف</label>
                <textarea name="description" class="db-input" rows="2">{{ old('description', $category->description) }}</textarea>
            </div>
            <div class="mb-4">
                <label>ترتيب العرض</label>
                <input type="number" name="sort_order" class="db-input" value="{{ old('sort_order', $category->sort_order) }}">
            </div>
            <div class="mb-4">
